...
is_project_env() {
  ...
  [[ -n "${env_path}" && "${env_path}" == "${PROJECT_DIR}"* ]]
}
...
start_backend() {
  ...
  if [[ -x "xtts-env/bin/python" ]] && python_has_modules "${PROJECT_DIR}/xtts-env/bin/python" fastapi uvicorn TTS.api; then
    PYTHON_BIN="${PROJECT_DIR}/xtts-env/bin/python"
    log "Using XTTS virtualenv: ${PROJECT_DIR}/xtts-env"
  elif [[ -n "${VIRTUAL_ENV:-}" && -x "${VIRTUAL_ENV}/bin/python" ]] && is_project_env "${VIRTUAL_ENV}"; then
    PYTHON_BIN="${VIRTUAL_ENV}/bin/python"
    log "Using active virtualenv: ${VIRTUAL_ENV}"
  elif [[ -n "${VIRTUAL_ENV:-}" && -x "${VIRTUAL_ENV}/bin/python" ]]; then
    log "Ignoring active virtualenv outside this project: ${VIRTUAL_ENV}"
